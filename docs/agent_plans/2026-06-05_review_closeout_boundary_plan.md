...
Restated concern: this repo owns `backlog-maintenance`, so its own active
backlog should dogfood that protocol by moving from `docs/protocol_backlog.md`
to a `docs/backlog.yml` registry — without leaving the live docs contradicting
themselves, without rewriting frozen review threads, and with a CI-backed
checker strong enough that the next agent does not have to guess.

Scope of this pass: the `## Backlog YAML Dogfood Amendment` section only
(plan lines 830-892). The review/closeout boundary and documentation
organization work above is already `ready for closeout`; this pass does not
reopen it.

What I verified directly:
- `docs/protocol_backlog.md` has exactly five open items (`### 1`-`### 5`), so
  the `AP-BL-0001`-`AP-BL-0005` mapping is one-to-one and correctly sized.
- The git remote is `resodo/agent-protocols`, so `repo: resodo/agent-protocols`
  is the correct registry slug.
- No `docs/backlog.yml`, `docs/backlog.md`, or `scripts/` exists yet, so the new
  files are greenfield.
- The repo has no YAML library, `requirements*.txt`, or dependency-install step;
  `.github/workflows/ci.yml` only runs stdlib `unittest` + `compileall` under
  `permissions: {}`.
- The validation `rg` at plan line 889 is broken: backticks inside the
  double-quoted pattern trigger shell command substitution. Reproduced —
  bash emits `docs/backlog.yml: No such file or directory` and drops the
  `Do not migrate...` alternative from the pattern entirely.

#### Blocking

**Thread 13 — The live-doc update must reverse the prior amendment's
anti-migration guidance, and the only check that would catch it is broken.**

This is the one place the amendment can leave the repo in the exact half-migrated,
self-contradictory state dogfooding is meant to remove, and it currently has no
working guard.

The immediately-prior Documentation Organization Amendment (already implemented
and live) deliberately added explicit anti-migration guidance to the live docs.
This amendment makes that guidance false:

- `AGENTS.md:44-46`: "Use `docs/protocol_backlog.md` for this repo's
  protocol-development backlog. Do not migrate this repo to `docs/backlog.yml`
  unless a future accepted plan explicitly adopts the backlog registry workflow
  for this repo." This amendment *is* that future accepted plan, so the escape
  clause is now satisfied and the whole prohibition must be removed, not just
  the pointer re-aimed.
- `docs/CURRENT.md:16` (and the `backlog-maintenance` line at 29-30),
  `docs/README.md:14`, `:33` (exceptions list), `:60` (maintenance trigger), and
  `README.md:25` all name `docs/protocol_backlog.md` as the live backlog.

Amendment item 3 only says "Update live indexes and entrypoints," which reads as
additive. If the implementer adds a `docs/backlog.yml` pointer but leaves the
prohibition, `AGENTS.md` will simultaneously route to `docs/backlog.yml` and
forbid migrating to it — a contradiction in the load-bearing entrypoint. The
acceptance ("Live docs point to `docs/backlog.yml`, not `docs/protocol_backlog.md`")
and the line-889 `rg` show the driver *intends* removal, including grepping for
the literal "Do not migrate this repo to `docs/backlog.yml`" string. But that
`rg` is the only check for this central property and it does not run: the
backticks inside the double-quoted pattern cause bash command substitution
(confirmed: it tries to execute `docs/backlog.yml` and silently drops the second
alternative). So as written, the plan has no executable guard for its own
central acceptance.

Recommended fix (small):
1. Make item 3 / acceptance explicit that the anti-migration guidance and all
   live `docs/protocol_backlog.md` pointers are removed or replaced across
   `AGENTS.md`, `README.md`, `docs/README.md`, and `docs/CURRENT.md` (the
   `docs/README.md` naming-exception entry and maintenance-trigger line too).
2. Fix the line-889 pattern (single-quote the `rg` pattern, or escape the
   backticks) and state the expected result: zero matches for
   `docs/protocol_backlog` / the prohibition string across the named live
   surfaces.

#### Non-blocking

**Thread 14 — Name how `check_backlog.py` parses YAML and how CI gets the
dependency.** Item 4's first hard check is "YAML parses," but the Python stdlib
has no YAML parser, and this repo has no YAML library, requirements file, or
dependency-install step — `ci.yml` runs only stdlib `unittest`/`compileall`. The
plan should name the approach: add `PyYAML` plus a `pip install pyyaml` step in
the new CI job, or parse a constrained YAML subset with stdlib only. Otherwise
the implementer guesses, and the obvious choice (`import yaml`) silently breaks
CI's currently dependency-free posture until an install step is added. This is
the one runner/CI-shape decision the focus's "is adding the checker plus CI
appropriate" question turns on; appropriate, yes, but the dependency path needs
to be settled in the plan.

**Thread 15 — The checker has no negative-case coverage; it only proves the
valid file passes.** The validation runs `python scripts/check_backlog.py`
against the real, valid backlog, which proves "valid passes" but not "malformed
fails." A checker that always exits 0 would sail through CI undetected — the
mechanism would look healthy while enforcing nothing (validation-first +
mechanism-lifecycle lens). This repo already keeps a `tests/` discipline for the
structured-review runner; the new checker has none planned. Recommend either a
minimal negative-case test (e.g. duplicate ID, bad `priority`, missing required
field, a stray `docs/backlog.md`) proving the checker actually rejects, or an
explicit deferral with rationale. Semantic quality (priority/kind/`why` value)
correctly stays review-owned per `backlog-maintenance/SKILL.md`; this is only
about proving the hard checks fire.

**Thread 16 — Schema and semantic-conversion details to pin down (minor).**
- Item 1 names `repo`, `id_prefix`, and `kinds`, but not the required top-level
  `version: 1`, which `backlog-maintenance/SKILL.md` lists in the schema and as
  a CI hard check ("top-level `version` ... exist"). Name it so it is not
  omitted.
- The five Markdown items carry no `priority` or `kind` and are not written as
  `why`/`next`/`done_when`. Converting them is net-new semantic authoring:
  `priority` and `kind` are fresh decisions not derivable from the source, and
  `why`/`next`/`done_when` are reshaped prose. The acceptance ("semantically
  faithful to the five existing open items") is therefore reviewer/human-judged
  provenance, not machine-checkable — worth saying so in the plan, and noting
  the SAFETY rule "when uncertain, do not choose P0."
- The generic CI hard-check list guards against an active `docs/backlog.md`, not
  `docs/protocol_backlog.md`. So the acceptance "`docs/protocol_backlog.md` is
  absent" is enforced only by the move itself (use `git rm`/`git mv`) and the
  live-surface sweep, not by the dogfooded checker. Also state the expected
  outcomes for the two `rg` sweeps (line 889 → empty over live surfaces once
  un-broken; line 890 → matches present) so the next agent can read pass/fail.

#### Overall judgment

Not ready to implement until Thread 13 is settled. The migration direction is
sound and genuinely dogfoods `backlog-maintenance`: the registry slug, ID
allocation, kinds, and source-of-truth move all follow the protocol, and a
five-to-five item mapping is correct. Historical-mention handling is good and
consistent with the prior Thread 9 precedent — the non-goals forbid rewriting
frozen review-thread bodies, the line-889 sweep is scoped to live surfaces
(named files + `structured-review` + `docs/agent_plans/README.md`, excluding the
dated plans), and this plan file itself is out of that scope, so the amendment's
own provenance mentions of `docs/protocol_backlog.md` are safely preserved. The
single genuine gap is that the central correctness property — live docs must stop
contradicting themselves after the migration — is left implicit in scope and its
only validation guard does not execute. Threads 14-16 are improvements that fold
into the same revision pass. Per the review/closeout boundary, this is an
`impl-plan` pass: it concludes `ready for implementation` once Thread 13 is
resolved, not a merge-readiness handoff.

#### Residual risks / validation gaps

- If Thread 13 is resolved by silence, the implementer may leave the
  "Do not migrate" prohibition in `AGENTS.md`, producing an entrypoint that both
  routes to and forbids `docs/backlog.yml`; the broken line-889 `rg` will not
  catch it.
- The YAML parser / CI-dependency choice (Thread 14) is unproven until a real CI
  run; treat the new job as untested until it executes with the chosen parser.
- Without a negative-case test (Thread 15), a silently-broken checker can pass
  CI while enforcing nothing.
- `priority`/`kind`/`why`/`next`/`done_when` for the five items are net-new
  semantic content; CI can check presence and format, not faithfulness — final
  acceptance is reviewer/human judgment.
